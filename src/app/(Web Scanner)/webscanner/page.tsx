...
export default function Home() {
    ...
    const [errorMessage, setErrorMessage] = useState<string>('Please, Check your Email & URL');
    ...
    const handleScan = async (event: any) => {
        ...
        try {
            ...
            const queryParams = await new URLSearchParams();
            await queryParams.append('url', url);
            await router.push(`/Result?${queryParams.toString()}`);
        } catch (error: any) {
            ...
            >
                ...
                >
                    ...
                    <div className="flex flex-col items-start p-[10px]">
                        <h1 className=" md:mb-14  mb-8 font-sans text-center text-white md:text-4xl text-3xl font-bold" >Website Vulnerability Scanner</h1>
                        <div className='font-mono text-white w-30 font-bold' >
                            Let VoltSec.io identify vulnerabilities in your website code. Our scanner helps keep your website safe from cyberattacks.
                        </div>
                        <div className='md:mt-14 mt-7 flex items-center justify-center flex-col w-full'>
                            ...
                            <input
                                type="text"
                                value={email}
                                onChange={(e) => setEmail(e.target.value)} className=" md:w-[45vw] lg:w-[45vw] px-4 py-2 border border-gray-300  focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 mt-5 rounded-xl "
